...
export function StartWorkoutForm() {
  ...
  return <form className="mb-6 rounded-2xl border border-slate-200 bg-white p-4 shadow-sm" onSubmit={async (event) => {
    ...
    <label className="grid min-w-0 gap-1.5 text-sm font-medium text-slate-600">タイトル（任意）
    ...
    <button disabled={pending} className="mt-3 w-full rounded-xl bg-orange-500 px-4 py-3 text-base font-semibold text-white shadow-sm disabled:opacity-50 active:bg-orange-600">{pending ? "開始中…" : "トレーニングを開始"}</button>
